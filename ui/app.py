...
with tab2:
    ...
    # --- WORKER: single date range (one output file per call) ---
    def ndvi_worker(
        job_id,
        fname,
        dataset_data,
    ...
    # --- WORKER: attribute-column date mode (one merged output file) ---
    def ndvi_column_worker(
        job_id,
        fname,
        dataset_data,
        date_column,
        ...
        try:
            ...
            for idx, target_date in enumerate(unique_dates):
                ...
                if result["status"] != "success":
                    print(
                        f"[NDVI column] {target_date} failed: {result.get('message')}"
                    )
                    continue
                ...
                with rasterio.open(tif_path) as src:
                    for row_idx, row in date_gdf.iterrows():
                        ...
                        try:
                            ...
                            ndvi_val = (
                                float(val[0][0]) if val.size > 0 else np.nan
                            )
                            if ndvi_val == -9999:
                                ndvi_val = np.nan
                        except Exception:
        ...
        if ndvi_input_datasets:
            ...
            for fname, d in ndvi_input_datasets.items():
                ...
                with st.expander(fname, expanded=True):
                    # --- Mode checkboxes (any combination allowed) ---
                    ...
                    if use_ranges:
                        ...
                        st.caption(
                            "One output file is produced per range."
                        )
                        remove_idx = None
                        for i, (s, e) in enumerate(cfg["ranges"]):
                            # Read currently stored values to pre-compute validity
                            # for the help tooltip (set before widget renders)
                        ...
                        if remove_idx is not None:
                            ...
                            for j in range(remove_idx, old_n):
                                st.session_state.pop(
                                    f"ndvi_rs_{fname}_{j}", None
                                )
                                st.session_state.pop(
                                    f"ndvi_re_{fname}_{j}", None
                                )
                            st.rerun()
                ...
                for fname, d in ndvi_input_datasets.items():
                    ...
                    if use_specific:
                        ...
                        for i, d_def in enumerate(
                            ...
                            start_d = target_date - timedelta(days=window_days)
                            # Cap the end of the window at today
                            end_d = min(
