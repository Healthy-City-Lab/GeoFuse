...
* **Pixel-aligned mosaic + clean edges**: Tiles are exported in a single planar CRS with a global snap grid, mosaicked in that CRS, and reprojected to WGS84 in one final pass. Adjacent tile boundaries align to the pixel. National-scale outputs still stream tile-by-tile so the mosaic step never has to fit the whole raster in RAM.
* **Sparse, multi-threaded WGS84 reproject**: The final mosaic → WGS84 step iterates destination blocks through a `WarpedVRT`, writes only blocks that contain valid pixels, and skips the rest entirely. `SPARSE_OK` keeps unwritten blocks off disk; they read back as nodata. The warper runs on every CPU core with a generous block cache. A buffered AOI costs roughly the populated pixels, not the full bounding box. Progress reports flushed (non-empty) block counts so the bracket counter tracks real work done.
...
* **Compressed tiled BIGTIFF outputs with overviews**: Every GeoTIFF the project writes uses DEFLATE compression with a float-aware predictor, internal tiling (256×256 for intermediates, 512×512 for the WGS84 mosaic), BIGTIFF support, and three internal overview levels (2×, 4×, 8×) for snappy zoom-out in Folium / QGIS without doubling write time on national rasters.
...
* **Two-band NDVI raster (NDVI + valid_obs)**: Every NDVI GeoTIFF is a two-band; Band 1 is the median NDVI, band 2 (`valid_obs`) is the per-pixel count of cloud-free observations that contributed to the composite. This lets you reject pixels with insufficient coverage in downstream analyses.
