...
- **Sized to the machine it runs on.** The stability search opens thread pools and the pre-aggregation build opens worker processes, both derived from the host's core count, so the same code fits a laptop or a compute node without edits; `GEOFUSE_WORKERS` overrides the width. Pre-aggregation gets processes because its per-pixel work is a long chain of small numpy calls that hold the interpreter lock, which no number of threads can spread across cores. Every run logs a stage-by-stage wall-clock breakdown and, per parallel phase, how many workers were actually busy — enough to tell a pool that is too small from work that will not divide.
