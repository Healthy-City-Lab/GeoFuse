...
* **Mixed-Effects / Longitudinal Mode**: When entities are measured at multiple time points, switch the fusion run into mixed-effects mode and score every trial via a linear mixed model (`statsmodels.MixedLM`) instead of the cross-sectional scorer. The form accepts either a **long-format target** (one row per `(entity, wave)` with an explicit wave column) or **wide / multi-file** (one target file per wave joined on a shared entity-id column), per-channel per-wave greenery files (with a "same file all waves" shortcut for channels that don't change over time), and a continuous **`years_since_baseline`** time variable derived from per-row dates (full date, year + month, year-only, and integer years all accepted). The MixedLM is `outcome ~ greenery + covariates + years_since_baseline + (1 + years_since_baseline | entity)` with toggles for the time fixed effect and the random-slope structure. Four scorers are available — `mixedlm_tstat` (default; |t-stat| of the greenery fixed effect), `mixedlm_marginal_r2` (Nakagawa marginal R² contribution), `mixedlm_lr` (likelihood-ratio vs the no-greenery null model), `mixedlm_coef` (signed coefficient). Optuna optimises the one you pick; after the study a **post-hoc reporting** stage re-scores all four metrics on every robust trial, every top-20 % trial that feeds the averaged composite, and the final composite itself, writing per-pool / per-trial / mean+95 % CI rows to `output_results/fusion/study_results/mixedlm_metrics.csv`. The pre-aggregation cache extends with a wave axis and dedupes storage when a channel reuses the same file across multiple waves.
