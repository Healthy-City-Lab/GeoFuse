...
- **Objective metrics.** **Distance correlation** (default) detects non-linear as well as linear associations and is unsigned, so a separate **direction** indicator is reported alongside it; **Spearman**, **R²**, **normalized RMSE**, and **mutual information** are also available.
